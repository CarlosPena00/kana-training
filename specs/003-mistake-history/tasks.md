...
- [X] T051 Verify the feature on at least one mobile target as the constitution's definition of done requires — build via `./run.sh preview` then `npm run android:aab` from `app/` — paying attention to the soft keyboard during a forced correction in the Romaji → Kana direction **[Done 2026-08-28 on a Galaxy S23, Android 15 / API 35. Verified: a mistake from an abandoned quiz survived an app kill; the Romaji → Kana correction card switched the soft keyboard to the Japanese kana IME and accepted る typed through it; the "write in Hiragana" label rendered; "Leave correction round" stayed reachable with the keyboard up, needing a short scroll once the correction panel appears. Found one defect — the feedback panel claimed "½ point" after a forced correction, contradicting a results screen that scored the same card 0/1. Fixed via FeedbackPanel's showAttemptCredit prop; the fix is covered by an e2e regression assertion but was NOT re-run on the device, which was disconnected before the rebuild.]**
